...
const Filter = ({ setfilter }: { setfilter :any}) => {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_IP_ADDRESS}:4000/api/product/Showfilter`)
      .then((response) => {
        if (!response.ok) {
  ...
  const handleFilterChange = (category: string, option: string) => {
    setSelectedFilters((prevData: any) => {
      prevData[category] = Array.isArray(prevData[category])
        ? prevData[category]
        : [];
      ...
      prevData[category] =
        prevData[category].length < 1 ? "" : prevData[category];
      return {
        ...prevData,
  ...
  return (
    <div className="p-4 w-fit h-[60vh] z-[1000] fixed top-24 left-[5%] bg-white rounded-2xl">
      ...
      <button
        onClick={QueryFilter}
        className="bg-blue-700 px-3 py-2 rounded-md text-white capitalize">
        filter
      </button>
    </div>
  );
